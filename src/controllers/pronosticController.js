  ...
  createPronostic: async (req, res) => {
    try {
      ...
      const limite = parseInt(
        dateTime.startOf("hour").format("HH:mm").split(":")[0]
      );
      console.log({ limite });
      ...
      for (let index = 0; index < arrayPronosticos.length; index++) {
        ...
        const weatherConditionsResult =
          await weatherConditionsServices.getWeatherConditionsByParameters(
            arrayPronosticos[index].temperature,
            arrayPronosticos[index].humidity,
            arrayPronosticos[index].pressure
          );
      ...
      if (HORAS_DIA + 1 == HORAS_DIA + 1 - limite) {
        ...
        Promise.all(
          pronosticos.map((pronostico) => Pronostic.create(pronostico))
        ).catch((error) => {
          console.error("Error insertar:", error);
        });
      } else {
